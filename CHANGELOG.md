...
- `/` in browse mode fuzzy-finds in the current listing. The query matches anywhere in a name, in order, so `ordcsv` finds `orders.csv`, and the listing narrows as you type with the matched characters highlighted. Matches are ranked: hits on word boundaries (`_`, `-`, `.`, `/`, a space, or a camelCase hump) and runs of consecutive characters score highest, gaps cost, and the shorter name wins a tie. A lowercase query is case-insensitive; a single uppercase character makes the whole query case-sensitive. `Enter` opens the highlighted match, `Esc` cancels and restores the full listing, the arrows or `ctrl-n` / `ctrl-p` move through the matches, and `ctrl-u` clears the query. No new dependency: the ranking is hand-rolled in `src/browser/find.rs`.
