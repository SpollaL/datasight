...
- `d` in browse mode saves the selected cloud object to a local file (#28). The prompt is pre-filled with the remote name and resolves the destination as you type, so the final path is on screen before you commit to it: a directory keeps the remote name, a leading `~` expands, and missing parent directories are created. An existing file is flagged before it is overwritten, and `Enter` / `Esc` are spelled out in the prompt. The key is offered only for `az://` and `s3://` listings — a local entry or a directory reports why instead of opening a prompt that cannot succeed — and a remote destination is refused before anything is written. A failed fetch leaves no partial file behind.
- `/` in browse mode fuzzy-finds in the current listing. The query matches anywhere in a name, in order, so `ordcsv` finds `orders.csv`, and the listing narrows as you type with the matched characters highlighted. Matches are ranked: hits on word boundaries (`_`, `-`, `.`, `/`, a space, or a camelCase hump) and runs of consecutive characters score highest, gaps cost, and the shorter name wins a tie. A lowercase query is case-insensitive; a single uppercase character makes the whole query case-sensitive. `Enter` opens the highlighted match, `Esc` cancels and restores the full listing, the arrows or `ctrl-n` / `ctrl-p` move through the matches, and `ctrl-u` clears the query. No new dependency: the matcher is about eighty lines in `src/browser/find.rs`.

### Fixed
- `q` quits browse mode from either pane. It was gated on no file being open, which left the browser pane with no way out once one was: the only exit was to `Tab` into the viewer and quit from there.
